...
class SearchAdapter(
    ...
    private val cells = ArrayList<SearchCell>()
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (holder) {
            is ItemViewHolder -> holder.bind(
                listener,
                (cells[position] as SearchCell.Item).item,
                (cells[position] as SearchCell.Item).ingredients
            )
    ...
    override fun getItemViewType(position: Int): Int {
        return when (cells[position]) {
            is SearchCell.Item -> VIEW_TYPE_ITEM
            is SearchCell.Skeleton -> VIEW_TYPE_SKELETON
            is SearchCell.Search -> VIEW_TYPE_SEARCH
            is SearchCell.Empty -> VIEW_TYPE_EMPTY
        }
    }

    fun setData(newCells: List<SearchCell>) {
        when {
            cells.isEmpty() -> {
            ...
            newCells.isEmpty() -> {
                ...
                val diffCallback = SearchDiffCallback(cells, newCells)
    ...
    override fun onBindViewHolder(
        ...
        if (holder is ItemViewHolder && payloads.firstOrNull() is SearchCell.Item) {
            holder.updateUI((payloads.first() as SearchCell.Item))
        } else {
    ...
    class ItemViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun updateUI(item: SearchCell.Item) {
